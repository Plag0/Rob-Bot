...
class League(commands.Cog):
  ...
  async def lol_passive(self, member, item, gamemode):
    ...
    if user["upgrade_gold_transmutation"][1] >= 1 and user["lol"]["summoner_name"] != "":
      ...
      percent = user["upgrade_gold_transmutation"][0]
      ...
      upgrade_level = user["upgrade_gold_transmutation"][1]
      upgrade_percent = user["upgrade_gold_transmutation"][0]
      ...
      if level_req[level] <= total_games - games:
        user["upgrade_gold_transmutation"][1] += 1
        level = user["upgrade_gold_transmutation"][1]
        percent = 1
        for i in range(level-1):
          percent += percent*.43
        percent = round(percent,1)
        user["upgrade_gold_transmutation"][0] = percent
        level_up = True
      ...
      if level_up == True:
        ...
        level_up_embed.add_field(name = f"Stats", value=f"Current Level: **+ {percent}%**\nNext Level: **+ {round(percent+percent*.43,1)}%** :lock: Play {level_req[level]} more games to unlock the next level!")
      ...
      embed.add_field(name = f":coin: Transmutation! +${amount:,}", value=f"You have a Gold Transmutation level of **{upgrade_level}**.\n**{upgrade_percent}%** of your gold has been transformed into Sloans.\nYour new balance is **${balance:,}**.")
  ...
  async def get_game_id(self, api_key, summoner_id, region):
    ...
    try:
      ...
      print(f"Failed to get game ID - Status Code: {status_code}")
